...
struct MemoryGame<CardContent> where CardContent: Equatable {
    ...
    private var indexOfTheOneAndOnlyOneFaceUpCard: Int? {
        ...
        set { // isFaceUp is only true if the index is the newValue
...
// add an extension to an array
// oneAndOnly return what is in the array
extension Array {
    var oneAndOnly: Element? {
        if count == 1 { // faceUpCardIndices.count -> self.count
            return first // faceUpCardIndices.first -> self.first
        } else {
            return nil
        }
    }
